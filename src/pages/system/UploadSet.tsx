...
const Index = (_props: any, ref: any) => {
    ...
    const tosFormRef = React.useRef<FormInstance>(null);
    ...
    const refresh = () => {
        getUploadConfigApi().then(res => {
            if (res.code === 200) {
                ...
                }else if (res.data.visible == 4) {
                    res.data.local.visible = true;
                }else if (res.data.visible == 5) {
                    res.data.tos.visible = true;
                }
                setInfo(res.data);
                // 七牛云
                ...
                //火山云
                tosFormRef.current?.setFieldsValue({
                    tos: res.data.tos,
                })
                // 本地
    ...
    const onFinish = (data: any) => {
        ...
        if (key === 'qiniu') {
            ...
            api=saveTosApi;
            obj.visible = obj.visible ? 5 : 0;
        }else{
            return message.error('配置错误');
        }
        api(obj).then(res => {
            if (res.code === 200) {
        ...
        <Row gutter={[16, 16]} >
            <Col span={6} xs={24} sm={24} md={24} lg={12} xl={12} xxl={6}>
                <div className=' bgbai  '>
                    ...
                    >
                        <Form.Item label='AccessKey' name={['qiniu', 'accessKey']} className='paddh'>
                            <Input placeholder='请输入AccessKey' />
                        </Form.Item>
                        <Form.Item label='SecretKey' name={['qiniu', 'secretKey']} className='paddh'>
                            <Input placeholder='请输入SecretKey' />
                        </Form.Item>
                        <Form.Item label='仓库名称' name={['qiniu', 'bucket']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
                        <Form.Item label='上传节点' name={['qiniu', 'endpoint']} className='paddh'>
                            <Input placeholder='请输入上传节点' />
                        </Form.Item>
            ...
            <Col span={6}  xs={24} sm={24} md={24} lg={12} xl={12} xxl={6}>
                <div className=' bgbai uploadSetItem' >
                    ...
                    >
                        ...
                        <Form.Item label='AccessKey' name={['txcos', 'sk']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
            ...
            <Col span={6}  xs={24} sm={24} md={24} lg={12} xl={12} xxl={6}>
                <div className=' bgbai uploadSetItem' >
                    ...
                    <Form
                        ...
                        onFinish={onFinish}
                    >
                        <Form.Item label='AccessKey' name={['tos', 'accessKey']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
                        <Form.Item label='SecretKey' name={['tos', 'secretKey']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
                        <Form.Item label='仓库名称' name={['tos', 'bucket']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
                        <Form.Item label='仓库地域' name={['tos', 'region']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
                        <Form.Item label='自定义域名' name={['tos', 'domain']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
                        <Form.Item label='使用状态' name={['tos', 'visible']} className='paddh' valuePropName='checked'>
                            <Switch checkedChildren='开启' unCheckedChildren='关闭' />
                        </Form.Item>
                        <div className='paddh flex flex_end paddb20'>
                            <Button type='primary' className='huibtn marginr12' onClick={() => reset('txcos')}>重置</Button>
            ...
            <Col span={6} xs={24} sm={24} md={24} lg={12} xl={12} xxl={6}>
                <div className=' bgbai uploadSetItem'>
                    ...
                    <Form
                        ...
                        onFinish={onFinish}
                    >
                        <Form.Item label='上传路径' name={['local', 'path']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
                        <Form.Item label='上传地址' name={['local', 'host']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
                        <Form.Item label='自定义域名' name={['local', 'domain']} className='paddh'>
                            <Input placeholder='请输入' />
                        </Form.Item>
